...
class RouteCollectionBuilderTest extends TestCase
{
    ...
    public function testAdd()
    {
        ...
        $addedRoute = $collectionBuilder->add('/checkout', 'WebStoreBundle:Order:checkout');
        $addedRoute2 = $collectionBuilder->add('/blogs', 'WebStoreBundle:Blog:list', 'blog_list');
        $this->assertInstanceOf('Symfony\Component\Routing\Route', $addedRoute);
        $this->assertEquals('WebStoreBundle:Order:checkout', $addedRoute->getDefault('_controller'));
    ...
    public function testFlushOrdering()
    {
        ...
        // 1) Add a route
        $routes->add('/checkout', 'WebStoreBundle:Order:checkout', 'checkout_route');
        // 2) Import from a file
        $routes->mount('/', $routes->import('admin_routing.yml'));
        // 3) Add another route
        $routes->add('/', 'WebStoreBundle:Default:homepage', 'homepage');
        // 4) Add another route
        $routes->add('/admin', 'WebStoreBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/admin', 'WebStoreBundle:Admin:dashboard', 'admin_dashboard');
        // add an unnamed route
        $collectionBuilder->add('/blogs', 'WebStoreBundle:Blog:list')
            ->setMethods(array('GET'));

        // integer route names are allowed - they don't confuse things
        $collectionBuilder->add('/products', 'WebStoreBundle:Product:list', 100);
